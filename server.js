...
const JWT_SECRET = process.env.JWT_SECRET || "SUPABASE_NOVO_SEGREDO_2026";
// ===========================
// FUNÇÃO DE LOG
// ===========================
async function registrarLog({
  titular_id = null,
  usuario_id = null,
  evento = "AÇÃO_DESCONHECIDA",
  status_enviado = "ATIVO",
  resultado = "SUCESSO",
  detalhe = null
}) {
  ...
  try {
    ...
    await pool.query(
      ...
      (titular_id, usuario_id, evento, status_enviado, resultado, detalhe)
      VALUES ($1,$2,$3,$4,$5,$6)`,
...
app.get("/titulares/:id/logs", async (req, res) => {
  try {
  ...
  SELECT
    l.data_hora AT TIME ZONE 'America/Sao_Paulo' AS data_hora,
    l.evento,
    l.status_enviado,
    l.resultado,
    l.detalhe,
...
app.post("/titulares", async (req, res) => {
  ...
  try {
    ...
    await registrarLog({
      ...
      evento: "CRIACAO_TITULAR",
      detalhe: "Titular cadastrado no sistema"
    });
...
app.get("/logs", authMiddleware, async (req, res) => {
  try {
    ...
    const titular_id = Number(req.query.titular_id);
    ...
    const r = await pool.query(`
      SELECT
        ...
        l.detalhe
      FROM logs_eventos l
      ${titular_id ? "WHERE l.titular_id = $1" : ""}
      ORDER BY l.data_hora DESC
      LIMIT 500
...
/* ===========================
   RENOVAR TITULAR
=========================== */
app.post("/titulares/:id/renovar", async (req, res) => {
  const id = Number(req.params.id);
  ...
  try {
    ...
    const meses = r.rows[0].tempo_contrato_meses;

    const novaVigencia = await pool.query(`
      SELECT
        CURRENT_DATE AS inicio,
        (CURRENT_DATE + ($1 || ' months')::interval)::date AS fim
    `,[meses]);

    const inicio = novaVigencia.rows[0].inicio;
    const fim = novaVigencia.rows[0].fim;
...
app.put("/titulares/:id/cancelar", async (req, res) => {
  ...
  try {
    ...
    await client.query("COMMIT");
      ...
      usuario_id: req.user?.id || null,
      evento: "CANCELAMENTO",
      detalhe: motivo || "Cancelamento manual"
    });
...
app.put("/titulares/:id/trocar-plano", async (req, res) => {
  ...
  try {
    ...
    const novo_plano_id = parseInt(req.body.novo_plano_id ?? req.body.plano_id);
    const motivo = req.body.motivo ? String(req.body.motivo).trim() : null;
    ...
    console.log("TROCA PLANO BODY:", req.body);
    ...
    FROM planos
    WHERE id IN ($1,$2)
    `, [plano_atual_id, novo_plano_id]);

    const planoAtual = planos.rows.find(p => p.id === plano_atual_id)?.titulo || plano_atual_id;
    const planoNovo = planos.rows.find(p => p.id === novo_plano_id)?.titulo || novo_plano_id;
    ...
    console.log("PLANO ATUALIZADO:", upd.rows);
    ...
    await registrarLog({
      titular_id,
      usuario_id: req.user?.id || null,
      evento: "TROCA DE PLANO",
      detalhe: `Plano ${planoAtual} → ${planoNovo}` + (motivo ? ` | Motivo: ${motivo}` : "")
    });
...
app.post("/titulares/:id/pagamentos", authMiddleware, requireRole("ADMIN","FINANCEIRO"), async (req, res) => {
  try {
    ...
    let pago_em = req.body.pago_em ? String(req.body.pago_em).slice(0,10) : null;
...
app.get("/financeiro/itens", authMiddleware, requireRole("ADMIN","FINANCEIRO"), async (req, res) => {
  try {
    await pool.query(`SELECT fn_verificar_inadimplencia();`);
    const q = String(req.query.q || "").trim();
...
app.post("/pagamentos/:id/estornar", authMiddleware, requireRole("ADMIN","FINANCEIRO"), async (req, res) => {
  try {
    ...
    const titularId = r.rows[0].titular_id;

    await registrarLog({
      titular_id: titularId,
      usuario_id: req.user?.id || null,
      evento: "ESTORNO",
      detalhe: "Pagamento estornado"
    });

    // Verifica se ainda existe pagamento pago
...
app.get("/auditoria", authMiddleware, requireRole("ADMIN"), async (req, res) => {
  try {

    const { inicio, fim, evento, usuario } = req.query;
    ...
    if (evento) {
      filtros.push(`l.evento = $${i++}`);
      valores.push(evento);
    }

    if (usuario) {
      filtros.push(`u.nome ILIKE $${i++}`);
...
app.get("/auditoria/exportar", async (req,res)=>{
...
if(usuario){
where.push(`usuario_nome ILIKE $${i++}`)
params.push(`%${usuario}%`)
}
...
const r = await pool.query(`
SELECT criado_em,usuario_nome,evento,titular_nome,detalhe
FROM logs_sistema
${w}
ORDER BY criado_em DESC
`,params)

let csv="Data,Usuario,Evento,Titular,Detalhe\n"

r.rows.forEach(l=>{
csv+=`${l.criado_em},${l.usuario_nome},${l.evento},${l.titular_nome},"${l.detalhe}"\n`
})
...
console.error(e)
res.status(500).json({ok:false,error:"Erro ao exportar auditoria"})
}
